...
type SemanticFunction = (...args: Node[]) => any;

export const sqlSemantics: {[key: string]: SemanticFunction} = {
	Exp(query, _) {
		return query.eval();
	},
	...
	Query(partial_query) {
		...
		return (table: Record<string, string | number>[]) => {
	...
	Partial_query(select_statement, from_statement, where_statement) {
		...
		return (table: Record<string, string | number>[], limit: number) => {
			let remaining = limit;
			...
			if (select_res[0] === "*") return res;
			else {
				return res.map((row) => {
					const res_row: Record<string, string | number> = {};
					select_res.forEach((key: string) => {
						res_row[key] = row[key];
					});
					return res_row;
				});
	...
	equality_comp_col(name_node, _space1, op_node, _space2, name_node_2) {
		...
		return (row: Record<string, string | number>) => {
			...
			if (operator === "=") return val === val_2;
			else return val !== val_2;
		};
	},
	...
	size_comp_col(name_node, _space1, op_node, _space2, name_node_2) {
		...
		return (row: Record<string, string | number>) => {
			...
			if (typeof val !== "number" || typeof val_2 !== "number") {
				throw new Error(
					"you cannot perform a comparison operation on a non-number: "
				);
			}
	...
	Cols_multiple(col, _, cols) {
		const res: string | string[] = cols.eval();

		let following_cols: string[];
		if (Array.isArray(res)) following_cols = res;
		else following_cols = [res];
